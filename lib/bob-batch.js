...
Batch.prototypes.getData = function( cProp, undefined ){
    if( cProp !== undefined ) {
        return this._data[ cProp ];
    }
...
Batch.prototypes.setData = function( prop, value ){
    if( typeof prop === 'object' ) {
        if( Array.isArray( prop ) ) {
            throw new Error( 'Cannot set array as raw bob data.' );
        }

        Object.assign( this._data, prop );

        return;
    }

    if( typeof prop !== 'string' ) {
        throw new Error( 'Cannot bob data property name must be a string.' );
    }
    ...
    var self = {
        ...
        addDependencies: function( cJobname, aDependencies, oOptions ) {
            ...
            for( i = 0; i < l; i++ ) {
                ...
                if( oJob ) {
                    if( oOptions.forEach ) {
                        // Determines if the processing continues.
                        lHalted = false;
        ...
        run: function(){
            ...
            if( !self._deferred ) {
                self._deferred = {resolve:null, reject:null};
                self._deferred.promise = new Promise(function(res,rej){
                    self._deferred.resolve = res;
                    self._deferred.reject = rej;
                });
            }
        ...
        runNow: function( deferred ){
            ...
            forEach( self.tasks, function( oCurrent, nextTask ){
                ...
                if( oPromise ){
                    oPromise.then( next ).catch( doFail );
                }
            })
            .then( function(){
